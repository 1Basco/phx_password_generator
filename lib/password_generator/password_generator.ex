defmodule PwGenerator do
  @moduledoc """
  Generates random password dependeing on the paramaters passed, Module main function is `generate(options)`.
  ...
  defp validate_length_is_integer(false, _options) do
    {:error, "Please provide a integer for length"}
  end
  ...
  defp validate_options_values_are_bool(false, _length, _options_without_length) do
    {:error, "Please provide a boolean for the options"}
  end
  ...
  defp validate_options(true, _length, _options) do
    {:error, "Only options allowed are number, uppercase and symbols"}
  end
